...
parser.add_argument("--train_image_dir", 
                    help="Location of train dataset.", type=str)
parser.add_argument("--val_image_dir", 
                    help="Location of val dataset.", type=str)
parser.add_argument("--train_annotations_file", 
                    help="Location of train annotations file.", type=str)
parser.add_argument("--val_annotations_file", 
                    help="Location of val annotations file.", type=str)
parser.add_argument("--output_dir", default='/tmp/',
                    help="Location of val annotations file.", type=str)
parser.add_argument("--min_area", default=96 * 48, type=int, 
                    help="Minimum area of bounding box. Smaller bounding boxes will be filtered out")
parser.add_argument("--object_name", default='person',
                    help="The VWW object, only vail in coco 80 classes.", type=str)
...
def create_tf_examples(image, annotations_list, image_dir, category_index, min_area,
                      wanted_category, f_r):
...
def _create_tf_record_from_coco_annotations(annotations_file, image_dir, output_dir, output_path, 
                                            min_area, wanted_category, num_shards):
  ...
  with contextlib2.ExitStack() as tf_record_close_stack, tf.gfile.GFile(annotations_file, 'r') as fid:
    ...
    f_r = open( os.path.join(output_dir, 'exam_obj.txt'), 'w') 
    ...
    for idx, image in enumerate(images):
      ...
      (_, tf_example, num_annotations_skipped, label) = create_tf_examples(
           image, annotations_list, image_dir, category_index, min_area, wanted_category, f_r)
...
def main(_):
  ...
  print('The output directory: {}'.format(args.output_dir))
  print('The finding object: {}'.format(args.object_name))
